...
function CategoryRow({ cat, maxPercentage }: { cat: DonorCategory; maxPercentage: number }) {
  ...
  return (
    <div>
      ...
      >
        <div className="flex items-center justify-between text-xs mb-1.5">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <span className="font-mono text-ink-3 tabular-nums">{formatMoney(cat.amount)}</span>
      ...
      {open && cat.topDonors && (
        <div className="mt-1 mb-2 ml-0 border border-border rounded overflow-hidden">
...
export function FundingSection({ politician }: FundingSectionProps) {
  const sourcesById = Object.fromEntries(
    politician.sources.map((s) => [s.id, s])
  )

  if (politician.funding.length === 0) return null

  return (
    <section className="mb-12">
      <h2 className="label-caps text-accent/70 mb-4">CAMPAIGN FUNDING</h2>

      <div className="space-y-6">
        {politician.funding.map((cycle) => {
          ...
          return (
            <div key={cycle.cycle} className="bg-surface border border-border rounded p-5">
              <div className="flex items-start justify-between mb-5">
                <div>
                  ...
                  <p className="font-mono text-xs text-ink-3 mt-0.5">
                    Total raised:{' '}
                    <span className="text-ink-2 font-semibold tabular-nums">{formatMoney(cycle.totalRaised)}</span>
                  </p>
                </div>
              ...
              <DataSourceFooter source="FEC.gov" freshness="recent" />
            </div>
          )
        })}
      </div>
