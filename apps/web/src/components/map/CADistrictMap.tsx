...
import { caMembersByDistrict } from '@/data/california-members'
...
export function CADistrictMap({ selectedDistrict, onDistrictHover, onDistrictClick }: Props) {
  ...
  useEffect(() => {
    fetch('/data/ca-districts.json')
      ...
      .then((data: FeatureCollection) => {
        ...
        // Large districts render first (SVG bottom) so small urban ones capture mouse events.
    ...
    <div className="relative bg-[#080d1a]">
      ...
      >
        ...
        {pathGen && geoData?.features.map(feature => {
          ...
          const member = caMembersByDistrict[num]
          const party  = (member?.party ?? 'D') as keyof typeof PARTY_COLORS
...
function coordCount(f: Feature): number {
  ...
  if (g.type === 'Polygon')
    return (g as { coordinates: number[][][] }).coordinates[0]?.length ?? 0
  if (g.type === 'MultiPolygon')
    return (g as { coordinates: number[][][][] }).coordinates.reduce((s, p) => s + (p[0]?.length ?? 0), 0)
  return 0
}
