...
import { flMembersByDistrict } from '@/data/fl-members'
...
export function FLDistrictMap({ selectedDistrict, onDistrictHover, onDistrictClick }: Props) {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_BASE_PATH ?? ''}/data/fl-districts.json`)
      .then(r => r.json())
    ...
    <div className="relative" style={{ background: mapBg }}>
      ...
      >
        ...
        {pathGen && geoData?.features.map(feature => {
          ...
          const member = flMembersByDistrict[num]
