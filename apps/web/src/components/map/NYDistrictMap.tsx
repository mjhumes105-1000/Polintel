...
import { nyMembersByDistrict } from '@/data/ny-members'
...
export function NYDistrictMap({ selectedDistrict, onDistrictHover, onDistrictClick }: Props) {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_BASE_PATH ?? ''}/data/ny-districts.json`)
      .then(r => r.json())
    ...
    <div className="relative" style={{ background: mapBg }}>
      ...
      >
        ...
        {pathGen && geoData?.features.map(feature => {
          ...
          const member = nyMembersByDistrict[num]
