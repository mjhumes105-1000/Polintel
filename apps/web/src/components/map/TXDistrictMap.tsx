...
import { txMembersByDistrict } from '@/data/tx-members'
...
export function TXDistrictMap({ selectedDistrict, onDistrictHover, onDistrictClick }: Props) {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_BASE_PATH ?? ''}/data/tx-districts.json`)
      .then(r => r.json())
    ...
    <div className="relative" style={{ background: mapBg }}>
      ...
      >
        ...
        {pathGen && geoData?.features.map(feature => {
          ...
          const member = txMembersByDistrict[num]
