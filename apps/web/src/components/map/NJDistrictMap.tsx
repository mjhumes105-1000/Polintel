...
import { njMembersByDistrict } from '@/data/nj-members'
...
export function NJDistrictMap({ selectedDistrict, onDistrictHover, onDistrictClick }: Props) {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_BASE_PATH ?? ''}/data/nj-districts.json`)
      .then(r => r.json())
    ...
    <div className="relative bg-[#080d1a]">
      ...
      >
        ...
        {pathGen && geoData?.features.map(feature => {
          ...
          const member = njMembersByDistrict[num]
